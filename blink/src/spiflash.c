...
void __not_in_flash_func(writeSuddenAmplitudeChangeRecordToFlash)(
    struct AmplitudeChangeTimerCallbackParameters *ac_params) {
    PRINTF("write sudden amplitude change record to flash\r\n");

    uint16_t sector_index = 0;
    uint8_t *flash_ac_records =
        (uint8_t *)(XIP_BASE + FLASH_AMPLITUDE_CHANGE_OFFSET);
    ...
    if (xSemaphoreTake(xFlashMutex, pdMS_TO_TICKS(250)) == pdTRUE) {
        ...
        for (data_index = 0; data_index < FLASH_AMPLITUDE_RECORDS_TOTAL_SECTOR *
                                              FLASH_SECTOR_SIZE;
             data_index += FLASH_SECTOR_SIZE) {
    ...
    // set vrms values
    memcpy(ac_flash_data.vrms_values_buffer, ac_params->vrms_values_buffer,
           ac_params->vrms_values_buffer_size_bytes);
    // set variance
    ...
    if (xSemaphoreTake(xFlashMutex, pdMS_TO_TICKS(250)) == pdTRUE) {
        ...
        if (sector_index == (FLASH_AMPLITUDE_RECORDS_TOTAL_SECTOR - 1)) {
            flash_range_erase(FLASH_AMPLITUDE_CHANGE_OFFSET +
        ...
        flash_range_program(FLASH_AMPLITUDE_CHANGE_OFFSET +
                                (sector_index * FLASH_SECTOR_SIZE),
                            (const uint8_t *)&ac_flash_data, FLASH_SECTOR_SIZE);
        restore_interrupts(ints);
        xSemaphoreGive(xFlashMutex);
    } else {
        PRINTF("MUTEX CANNOT RECEIVED!\r\n");
